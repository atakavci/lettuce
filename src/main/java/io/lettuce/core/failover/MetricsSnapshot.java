...
public class MetricsSnapshot {
    ...
    /**
     * Timestamp (milliseconds) when this snapshot was created.
     */
     ...
     */
    public MetricsSnapshot(long successCount, long failureCount) {
        this(successCount, failureCount, System.currentTimeMillis());
    }

    /**
     * Create a new metrics snapshot with the given counts and timestamp.
     *
     * @param successCount the number of successful commands
     * @param failureCount the number of failed commands
     * @param snapshotTime the timestamp when this snapshot was created
     */
    public MetricsSnapshot(long successCount, long failureCount, long snapshotTime) {
     ...
     *
     * @return the snapshot time in milliseconds
     */
    ...
    /**
     * Check if this snapshot has any data (at least one command executed).
     *
    ...
    /**
     * Check if this snapshot indicates a healthy state (no failures).
     *
     * @return true if failure count is 0
     */
    ...
    public String toString() {
        return "MetricsSnapshot{" + "success=" + successCount + ", failure=" + failureCount + ", total=" + getTotalCount()
                + ", failureRate=" + String.format("%.2f", getFailureRate()) + "%, snapshotTime=" + snapshotTime + '}';
    }
