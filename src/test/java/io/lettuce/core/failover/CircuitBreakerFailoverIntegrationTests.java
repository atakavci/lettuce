...
class CircuitBreakerFailoverIntegrationTests extends AbstractRedisClientTest {
    ...
    void setUp() {
        ...
        CircuitBreaker.CircuitBreakerConfig cbConfig = new CircuitBreaker.CircuitBreakerConfig(10.0f, // 10% failure rate
                                                                                                      // threshold
                3, // Only need 3 failures minimum (instead of default 1000)
    ...
    void shouldAutomaticallyFailoverWhenCircuitBreakerOpens() throws Exception {
        ...
        CircuitBreakerStateListener listener = event -> {
            if (event.getNewState() == CircuitBreaker.State.OPEN) {
                // Give time for failover to happen
                try {
                    Thread.sleep(500);
                    failedOverTo.set(connection.getCurrentEndpoint());
    ...
    void shouldTrackFailuresInCircuitBreakerMetrics() {
        ...
        long initialFailures = cb.getMetrics().getSnapshot().getFailureCount();
        ...
        long finalFailures = cb.getMetrics().getSnapshot().getFailureCount();
        assertThat(finalFailures).isGreaterThan(initialFailures);
